...
export async function GET(
  ...
  const { data: order, error } = await admin
    ...
    .select(`
      ...
      order_items(id, release_id, title, artist, price, quantity, thumb)
    `)
    .eq('id', id)
    .single()
...
export async function PATCH(
  ...
  const ALLOWED_STATUS = ['pending', 'paid', 'confirmed', 'processing', 'shipped', 'delivered', 'collected', 'cancelled', 'refunded']
  ...
  // If collected or cancelled, update release statuses
