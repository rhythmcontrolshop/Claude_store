...
export default function AgendaManager() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      else setErrorMsg(data.error || 'Error al cargar eventos')
    } catch { setErrorMsg('Error de conexión al cargar eventos') }
    setLoading(false)
  }

  const resetForm = () => { setForm({ date: '', type: 'DJ SET', title: '', venue: '', lineup: '', flyer_url: '', web: '' }); setEditing(null); setMsg(null); setErrorMsg(null) }

  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) { const reader = new FileReader(); reader.onloadend = () => { setForm(f => ({ ...f, flyer_url: reader.result as string })) }; reader.readAsDataURL(file) }
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true); setMsg(null); setErrorMsg(null)
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) { setMsg(`Evento ${editing ? 'actualizado' : 'creado'}`); resetForm(); fetchEvents() }
  ...
  const handleEdit = (event: Event) => {
    setEditing(event); setForm({ date: event.date, type: event.type, title: event.title, venue: event.venue, lineup: event.lineup?.join(', ') || '', flyer_url: event.flyer_url || '', web: event.web || '' }); window.scrollTo(0, 0)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      else { const data = await res.json().catch(() => ({ error: 'Error al borrar' })); setErrorMsg(data.error || 'Error al borrar') }
    } catch { setErrorMsg('Error de conexión al borrar') }
  }
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto">
      ...
      {errorMsg && <div className="mb-4 p-3" style={{ border: '1px solid #ef4444', backgroundColor: '#fef2f2' }}><p className="text-xs" style={{ color: '#ef4444' }}>{errorMsg}</p></div>}
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 p-6" style={{ border: '1px solid #d1d5db' }}>
        ...
        <div className="col-span-full">
          ...
          {form.flyer_url && <div className="mt-2 p-2 inline-block" style={{ border: '1px solid #e5e7eb' }}><img src={form.flyer_url} alt="Preview" className="w-24 h-24 object-cover" /></div>}
        </div>
        ...
        <div className="col-span-full flex gap-4 items-center">
          <button type="submit" disabled={saving} className="text-xs px-6 py-2 transition-colors disabled:opacity-50 hover:opacity-90" style={{ backgroundColor: '#000000', color: '#FFFFFF' }}>{saving ? 'GUARDANDO...' : (editing ? 'ACTUALIZAR' : 'CREAR')}</button>
          {editing && <button type="button" onClick={resetForm} className="text-xs px-6 py-2 hover:bg-gray-100 transition-colors" style={{ border: '1px solid #d1d5db', color: '#374151' }}>CANCELAR</button>}
        </div>
      </form>
      <section>
        <h3 className="text-lg font-bold mb-4" style={{ color: '#000000' }}>EVENTOS GUARDADOS</h3>
        {loading ? <p className="text-xs animate-pulse" style={{ color: '#6b7280' }}>Cargando...</p> : events.length === 0 ? (
          <div className="p-4" style={{ border: '1px solid #e5e7eb' }}><p className="text-xs" style={{ color: '#6b7280' }}>No hay eventos. Crea el primero arriba.</p></div>
        ) : (
          <div className="overflow-x-auto" style={{ border: '1px solid #d1d5db' }}>
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead><tr style={{ borderBottom: '2px solid #000000' }}>
              ...
              <tbody>{events.map(ev => (
                <tr key={ev.id} className="hover:bg-gray-50" style={{ borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <td className="p-2"><p className="text-sm font-bold uppercase" style={{ color: '#000000' }}>{ev.title}</p><p className="text-xs" style={{ color: '#6b7280' }}>{ev.type} @ {ev.venue}</p></td>
                  <td className="p-2">{ev.flyer_url && <img src={ev.flyer_url} alt="Flyer" className="w-10 h-10 object-cover" style={{ border: '1px solid #d1d5db' }} />}</td>
                  <td className="p-2 text-right space-x-2">
                    <button onClick={() => handleEdit(ev)} className="text-xs px-3 py-1 hover:bg-black hover:text-white transition-colors" style={{ border: '1px solid #d1d5db', color: '#374151' }}>EDITAR</button>
                    <button onClick={() => handleDelete(ev.id)} className="text-xs px-3 py-1 hover:bg-red-500 hover:text-white transition-colors" style={{ border: '1px solid #ef4444', color: '#ef4444' }}>X</button>
                  </td>
                </tr>
              ))}</tbody>
            </table>
