...
export default async function ShippingPage() {
  ...
  try {
    ...
    if (error) { console.error('Shipping rates fetch error:', error); fetchError = error.message }
    else { rates = data as ShippingRateRow[] }
  } catch (err) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6" style={{ borderBottom: '1px solid #e5e7eb', paddingBottom: '1rem' }}>
        ...
        <p className="text-xs" style={{ color: '#6b7280' }}>{rates?.length ?? 0} tarifas configuradas</p>
      </div>
      {fetchError && (
        <div className="p-4 mb-6" style={{ border: '1px solid #ef4444', backgroundColor: '#fef2f2' }}>
          ...
          <p className="text-xs mt-2" style={{ color: '#6b7280' }}>Verifica que la tabla "shipping_rates" existe en Supabase y que las variables de entorno están configuradas.</p>
        </div>
      )}
            ...
            <tbody>
              {rates.map(rate => (
                <tr key={rate.id} className="hover:bg-gray-50" style={{ borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <td className="px-3 py-3 text-xs" style={{ color: '#000000' }}>{ZONE_LABELS[rate.zone] || rate.zone}</td>
                  <td className="px-3 py-3 text-xs" style={{ color: '#000000' }}>{METHOD_LABELS[rate.method] || rate.method}</td>
                  <td className="px-3 py-3 text-xs" style={{ color: '#6b7280' }}>{rate.carrier || '—'}</td>
                  <td className="px-3 py-3 text-xs" style={{ color: '#000000' }}>{rate.min_weight_kg} - {rate.max_weight_kg}</td>
                  <td className="px-3 py-3 text-sm font-bold" style={{ color: '#000000' }}>{rate.price.toFixed(2)} €</td>
                  <td className="px-3 py-3 text-xs" style={{ color: '#6b7280' }}>{rate.free_above ? `${rate.free_above.toFixed(2)} €` : '—'}</td>
                  <td className="px-3 py-3">
                    <span className="text-xs px-2 py-1" style={{ color: rate.is_active ? '#22c55e' : '#9ca3af', border: `1px solid ${rate.is_active ? '#22c55e' : '#9ca3af'}` }}>
